...
// In-memory cache: "lat,lng,zoom" → { data, timestamp }
const cache = new Map()
const CACHE_TTL = 15 * 60 * 1000

function getBbox(centerLat, centerLng, zoom) {
  // Degrees of lng to show, halved for each zoom step above 10
  const radiusLng = 0.8 / Math.pow(2, zoom - 10)
  // Latitude radius accounts for Mercator compression
...
(
  ...
  way["landuse"~"^(park|recreation_ground|forest|grass|meadow|village_green|greenfield|allotments)$"](${b});
  way["leisure"~"^(park|garden|nature_reserve|pitch|golf_course)$"](${b});
  way["natural"~"^(wood|scrub|grassland|wetland)$"](${b});
...
export function parseOSMData(rawData) {
  ...
  for (const el of rawData.elements) {
    ...
    if (t.highway) {
      ...
      else if (h === 'footway' || h === 'cycleway' || h === 'path' || h === 'pedestrian' || h === 'steps') roads.path.push(coords)
...
export async function fetchOSMData(centerLat, centerLng, zoom) {
  const key = `${centerLat.toFixed(3)},${centerLng.toFixed(3)},${Math.round(zoom)}`
  const hit = cache.get(key)
  if (hit && Date.now() - hit.timestamp < CACHE_TTL) return hit.data

  const bbox = getBbox(centerLat, centerLng, zoom)
  const query = buildQuery(bbox, zoom)
